...
export default function PanelGestionTiendas({ tiendas }: PanelGestionTiendasProps) {
  ...
  const suspenderTienda = async (tiendaId: string) => {
    ...
    if (error) {
      ...
      alert('Tienda suspendida exitosamente')
  ...
  const activarTienda = async (tiendaId: string) => {
    ...
    if (error) {
      ...
      alert('Error al activar tienda')
    } else {
      alert('Tienda activada exitosamente')
      window.location.reload()
    }
  }

  const renovarLicencia = async (tiendaId: string) => {
    const { error } = await supabase
      .from('Tienda')
      .update({
        licenciaHasta: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString()
      })
      .eq('id', tiendaId)
    ...
    if (error) {
      ...
      alert('Licencia renovada por 1 año')
      window.location.reload()
    }
  }
